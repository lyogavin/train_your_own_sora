        ...
        if self.loss_type == LossType.KL or self.loss_type == LossType.RESCALED_KL:
            ...
            if self.model_var_type in [
                ...
                if model.model.training:
                    x_t = rearrange(x_t, 'b c f h w -> b f c h w', b=x_t.shape[0]).contiguous()
                    x_start = rearrange(x_start, 'b c f h w -> b f c h w', b=x_start.shape[0]).contiguous()
                    model_output = rearrange(model_output, 'b c f h w -> b f c h w', b=x_t.shape[0]).contiguous()

                B, F, C = x_t.shape[:3]
                #print(f"model_output: {model_output}")
                assert model_output.shape == (B, F, C * 2, *x_t.shape[3:]), f"model_output.shape: {model_output.shape}, x_t.shape: {x_t.shape}"
                model_output, model_var_values = th.split(model_output, C, dim=2)
            ...
            if model.model.training:
                target = rearrange(target, 'b c f h w -> b f c h w', b=target.shape[0]).contiguous()
