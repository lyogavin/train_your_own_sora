...
class VideoGenPipeline(DiffusionPipeline):
    ...
    def __init__(
        ...
        #print(f"pipeline dump models: text_encoder:{(str(text_encoder.state_dict())[:200], str(text_encoder.state_dict())[-200:])}")

        #print(f"pipeline dump models: vae:{(str(vae.state_dict())[:200], str(vae.state_dict())[-200:])}")

        #print(f"pipeline dump models: transformer:{(str(transformer.state_dict())[:200], str(transformer.state_dict())[-200:])}")
    ...
    def __call__(
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                #print(f"latent_model_input dtypes: {latent_model_input.dtype}")
                #print(f"prompt_embeds dtypes: {prompt_embeds.dtype}")
